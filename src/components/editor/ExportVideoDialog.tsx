...
export const ExportVideoDialog = () => {
  ...
  const renderThumbnail = async (ctx: CanvasRenderingContext2D, canvas: HTMLCanvasElement) => {
    // Pegar a primeira imagem dos clips (mesma lógica do preview)
    const firstImageClip = clips
      .filter((c) => c.type === 'image' && c.track.startsWith('V'))
    ...
    // Overlay unificado — MESMO renderer usado pelo preview
    drawThumbnailOverlay({ ctx, canvas, data: thumbnailData });

    // Reset shadow (drawThumbnailOverlay já faz, mas garantimos)
